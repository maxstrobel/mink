...
def get_subtree_joints(model: mujoco.MjModel, body_id: int) -> list[int]:
    ...
    """
    robot_body_ids = set(get_subtree_body_ids(model, body_id))
    return [id for id in range(model.njnt) if model.jnt_bodyid[id] in robot_body_ids]
